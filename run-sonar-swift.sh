...
function runCommand() {
	...
	if [ "$nflag" = "on" ]; then
		...
		if [[ $returnValue != 0 && $returnValue != 127 ]] ; then
			stopProgress
...
if [ "$oclint" = "on" ] && [ "$hasObjC" = "yes" ]; then
	...
	while read word; do
	    ...
	    runCommand no oclint-json-compilation-database $includedCommandLineFlags -- -disable-rule=UselessParentheses -disable-rule=UseObjectSubscripting -disable-rule=UseNumberLiteral -disable-rule=UseContainerLiteral -disable-rule=UseBoxedExpression -disable-rule=UnnecessaryNullCheckForDealloc -disable-rule=UnnecessaryElseStatement -disable-rule=UnnecessaryDefaultStatement -rc=TooManyMethods=50 -disable-rule=MissingDefaultStatement -disable-rule=RedundantNilCheck -disable-rule=PreferEarlyExit -rc=TOO_MANY_FIELDS=50 -disable-rule=ParameterReassignment -rc=LONG_VARIABLE_NAME=40 -rc=LONG_METHOD=100 -disable-rule=NPATH_COMPLEXITY -rc=NCSS_METHOD=100 -rc LONG_LINE=$longLineThreshold -max-priority-1 $maxPriority -max-priority-2 $maxPriority -max-priority-3 $maxPriority -report-type pmd -o sonar-reports/$(echo $word | sed 's/\//_/g')-oclint.xml
